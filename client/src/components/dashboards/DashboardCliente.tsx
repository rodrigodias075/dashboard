...
const CORES_CLIENTES = [
  "oklch(0.60 0.28 140)", "oklch(0.65 0.30 30)", "oklch(0.62 0.25 20)",
  "oklch(0.58 0.26 240)", "oklch(0.68 0.27 60)", "oklch(0.52 0.24 180)",
  "oklch(0.64 0.26 320)", "oklch(0.56 0.25 100)", "oklch(0.61 0.27 200)",
  "oklch(0.59 0.29 270)", "oklch(0.63 0.24 90)", "oklch(0.57 0.27 150)",
  "oklch(0.66 0.26 45)", "oklch(0.54 0.25 210)", "oklch(0.62 0.28 300)",
];
...
export default function DashboardCliente() {
  ...
  // Agregar por Cliente
  const dadosPorCliente = dadosFiltrados.reduce((acc, item) => {
    if (!acc[item.cliente]) {
      acc[item.cliente] = { cliente: item.cliente, meses: {}, total_qtde: 0, total_valor: 0 };
    }
    ...
    item.meses.forEach(mes => {
      ...
      if (!acc[item.cliente].meses[mes.mes]) {
        acc[item.cliente].meses[mes.mes] = { mes: mes.mes, qtde: 0, valor: 0 };
      }
      acc[item.cliente].meses[mes.mes].qtde += mes.qtde;
      acc[item.cliente].meses[mes.mes].valor += mes.valor;
      acc[item.cliente].total_qtde += mes.qtde;
      acc[item.cliente].total_valor += mes.valor;
    });
    
    return acc;
  }, {} as Record<string, any>);

  const clientesData = Object.values(dadosPorCliente);
  const totalVendas = clientesData.reduce((acc: number, c: any) => acc + c.total_valor, 0);
  const totalItens = clientesData.reduce((acc: number, c: any) => acc + c.total_qtde, 0);
  const mediaTicket = totalItens > 0 ? totalVendas / totalItens : 0;

  // Ranking de clientes (Top 10)
  const rankingClientes = [...clientesData]
    .sort((a: any, b: any) => b.total_valor - a.total_valor)
    .slice(0, 10)
    .map((c: any, idx) => ({
      cliente: c.cliente && c.cliente.length > 25 ? c.cliente.substring(0, 25) + '...' : c.cliente || 'Cliente',
      valor: c.total_valor,
      cor: CORES_CLIENTES[idx % CORES_CLIENTES.length]
    }));
  ...
  const evolucaoMensal = (() => {
    ...
    return mesesFiltrados.map(mes => {
      const valorTotal = clientesData.reduce((acc: number, cliente: any) => {
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="glow-effect bg-gradient-to-br from-destructive/10 to-transparent border-destructive/20 hover:border-destructive/40 transition-all duration-300">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="font-label text-xs uppercase text-muted-foreground">Clientes</CardTitle>
              <Users className="w-5 h-5 text-destructive" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="font-data text-2xl text-foreground">{clientesData.length}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="evolucao" className="w-full">
                ...
                <LineChart data={evolucaoMensal}>
                  <defs>
                    <linearGradient id="colorValor2" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="oklch(0.60 0.28 140)" stopOpacity={0.8}/>
                      <stop offset="95%" stopColor="oklch(0.60 0.28 140)" stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Line 
                    type="monotone" 
                    dataKey="valor" 
                    stroke="oklch(0.60 0.28 140)" 
                    strokeWidth={3}
                    dot={{ fill: 'oklch(0.60 0.28 140)', r: 5 }}
                    activeDot={{ r: 7 }}
                  />
                </LineChart>
              </ResponsiveContainer>
        ...
        <TabsContent value="ranking" className="mt-6">
          <Card className="glow-effect">
            <CardHeader>
              <CardTitle className="font-display text-xl">Top 10 Clientes</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={rankingClientes} layout="vertical">
                  ...
                  <YAxis type="category" dataKey="cliente" tick={{ fontSize: 11 }} stroke="oklch(0.50 0.02 280)" width={150} />
                  <Tooltip contentStyle={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px' }} formatter={formatarTooltip} />
                  <Bar dataKey="valor" radius={[0, 8, 8, 0]}>
                    {rankingClientes.map((entry, index) => (
